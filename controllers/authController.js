...
const resetFunc = (token) => {
    ...
    const hashToken = crypto.createHash('sha256').digest('base64').toString('hex')
...
const signUp = catchAsync(async (req, res, next) => {
    const { first_name, last_name, username, email, phone, password, passwordConfirm } = req.body
    const user = await User.create({ first_name, last_name, username, email, phone, password, passwordConfirm })
    if (!user) {
...
const updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!currentPassword || !password || !passwordConfirm || !email) {
        return next(new AppError('Fields currentpassword,password, email required', 400))
    }
    //email bilan tekshirdim shu kishi bor yo'qligini
    const user = await User.findOne({
        attributes: ['id', 'email', 'password', 'username', 'role', 'passwordConfirm'],
        where: {
            email,
        },
    })
